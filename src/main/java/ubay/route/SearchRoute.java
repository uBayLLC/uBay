    ...
    private List<Item> getItems(Request req){
        ...
        try {
            ...
            if (!itemName.equalsIgnoreCase("") && !filter.equalsIgnoreCase("")) {
                preparedStatement = con.prepareStatement("SELECT item.item_id, item.name, item.description, item.photo, item.seller_id, item.tag_id FROM item JOIN tag ON tag.tag_id = item.tag_id WHERE tag.name LIKE '%" + filter + "%' AND item.name LIKE '%" + itemName + "%'");
